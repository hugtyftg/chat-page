      ...
      <div className="w-full max-x-4xl mx-auto flex flex-col items-center px-4 space-y-4">
        ...
        {messageList.length !== 0 &&
          (streamingId !== '' ? (
            ...
            <Button icon={MdRefresh} variant="primary" onClick={reSend}>
              重新生成
            </Button>
          ))}
        {/* 底部图标输入框和按钮 */}
        <div className="flex items-end w-full border border-black/10 dark:border-gray-800/50 bg-white dark:bg-gray-700 rounded-lg shadow-[0_0_15px_rgba(0,0,0,0.1)] py-4">
          ...
          <ReactTextareaAutosize
            className="flex-1 max-h-64 mb-0 bg-transparent text-black dark:text-white resize-none border-0 p-2"
            placeholder="输入一条消息..."
            rows={1}
            // React手动实现MVVM
            value={messageText}
